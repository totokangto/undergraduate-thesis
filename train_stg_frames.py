...
def training_one_frame(dataset, opt, pipe, load_iteration, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, frame_num, output_path):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        loss/=opt.batch_size
        loss.backward() # 각 parameter tensor의 .grad에 기울기 저장
        iter_end.record()
        ...
        with torch.no_grad():
            ...
            # Optimizer step
            if iteration < opt.iterations:
                gaussians.ntc_optimizer.step() # 기울기를 바탕으로 parameter update
                gaussians.ntc_optimizer.zero_grad(set_to_none = True) # 기울기 0으로 초기화
    ...
    progress_bar = tqdm(range(opt.iterations, opt.iterations + iterations_s2), desc="Training progress of Stage 2")
    
    # Train another NTC for fast object 
    for iteration in range(opt.iterations + 1, opt.iterations + iterations_s2 + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}"})
                progress_bar.update(10)
            if iteration == opt.iterations + iterations_s2:
                progress_bar.close()
            
            # Log and save
            s2_ress = training_report(tb_writer, iteration, Ll1, Lds, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
            if s2_ress is not None:
                last_s2_res.append(s2_ress)
            ...
            if iteration > 230: 
                ...
                viewspace_point_tensor_grad = viewspace_point_tensor.grad[:gaussians._xyz.shape[0]]
                gaussians.max_radii2D[visibility_filter] = torch.max(gaussians.max_radii2D[visibility_filter], radii[visibility_filter])
                # get view-space gradient 
                gaussians.add_densification_stats(viewspace_point_tensor_grad, visibility_filter)
    ...
    # get whole 3DGs
    gaussians.concat_gaussians()
    
    # save ply of stage 2 
    print("\n[ITER {}] Saving Gaussians".format(opt.iterations + iterations_s2))
    scene.save(iteration=opt.iterations + iterations_s2, save_type='all')

    # Stage 3
    gaussians.training_one_frame_s3_setup(opt)
    progress_bar = tqdm(range(opt.iterations + iterations_s2, opt.iterations + iterations_s2 + opt.iterations_s2), desc="Training progress of Stage 3")    
    
    # Train the added Gaussians    
    for iteration in range(opt.iterations + iterations_s2 + 1, opt.iterations + iterations_s2 + opt.iterations_s2 + 1):        
        iter_start.record()
        ...
        with torch.no_grad():
            ...
            if (iteration - opt.iterations) % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}"})
                progress_bar.update(10)
            if iteration == opt.iterations + iterations_s2 + opt.iterations_s2:
                progress_bar.close()

            # Log and save
            s3_res = training_report(tb_writer, iteration, Ll1, Lds, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
            if s3_res is not None:
                last_s3_res.append(s3_res)
            if (iteration in saving_iterations):
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                scene.save(iteration=iteration, save_type='all')
                      
            # Densification
            if (iteration - opt.iterations - iterations_s2) % opt.densification_interval == 0: # every 20 iters
    ...
    if iteration in testing_iterations: # only test at 150, 250, and 350 iters
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {}".format(iteration, config['name'], l1_test, psnr_test, psnr_of))
                if tb_writer:
