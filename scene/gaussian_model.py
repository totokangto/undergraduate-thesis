...
class GaussianModel:
    ...
    @property
    def get_temp_xyz(self):
        if self._temp_xyz is not None:
            return self._temp_xyz
        else:
            return None
    ...
    # stage 1.5 : train only for fast 3DG
    ...
    def training_one_frame_s2_setup(self,training_args):
        ...
        mask = (contracted_xyz >= 0) & (contracted_xyz <= 1)
        mask = mask.all(dim=1)    
        #selected_pts_mask = torch.where((torch.norm(grads, dim=-1) >= 0.0003) & (torch.norm(grads, dim=-1) <= 0.0004), True, False) # 0.0002
        selected_pts_mask = torch.where((torch.norm(grads, dim=-1) >= 0.0003), True, False) # 0.0002
        selected_pts_mask = torch.logical_and(selected_pts_mask, mask)

        # 다른 3DG들은 temp에 따로 저장
        # 렌더링하기 전에 concat되는데 사용되거나 stage 2 시작 전에 concat되는데 사용됨 
        self._temp_xyz = self._xyz[~selected_pts_mask].detach()
        self._temp_features_dc = self._features_dc[~selected_pts_mask].detach()
        self._temp_features_rest = self._features_rest[~selected_pts_mask].detach()
        self._temp_scaling = self._scaling[~selected_pts_mask].detach()
        self._temp_rotation = self._rotation[~selected_pts_mask].detach()
        self._temp_opacity = self._opacity[~selected_pts_mask].detach()
        
        self.temp_xyz_gradient_accum = self.xyz_gradient_accum[~selected_pts_mask].detach()
        self.temp_color_gradient_accum = self.color_gradient_accum[~selected_pts_mask].detach()
        self.temp_max_radii2D = self.max_radii2D[~selected_pts_mask].detach()
        self.temp_denom = self.denom[~selected_pts_mask].detach()
 
        # grad 높은 3DG만 저장한 후 추가 학습
        ...
        self.color_gradient_accum = torch.zeros((self._xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self._xyz.shape[0], 1), device="cuda")
        self.max_radii2D = torch.zeros((self._xyz.shape[0]), device="cuda")
        torch.cuda.empty_cache()
 
        
    def rendering_gaussians(self):     
        xyz = torch.concat((self.get_xyz, self._temp_xyz),dim=0)
        features_dc = torch.concat((self._features_dc, self._temp_features_dc),dim=0)
        features_rest = torch.concat((self._features_rest, self._temp_features_rest),dim=0)
        scaling = torch.concat((self._scaling, self._temp_scaling),dim=0)
        rotation = torch.concat((self.get_rotation, self._temp_rotation),dim=0)
        opacity = torch.concat((self._opacity, self._temp_opacity),dim=0)
        
        return xyz, features_dc, features_rest, scaling, rotation, opacity
        
    def concat_gaussians(self):
        self._xyz = torch.concat((self._xyz, self._temp_xyz),dim=0)
        self._features_dc = torch.concat((self._features_dc, self._temp_features_dc),dim=0)
        self._features_rest = torch.concat((self._features_rest, self._temp_features_rest),dim=0)
        self._scaling = torch.concat((self._scaling, self._temp_scaling),dim=0)
        self._rotation = torch.concat((self._rotation, self._temp_rotation),dim=0)
        self._opacity = torch.concat((self._opacity, self._temp_opacity),dim=0)
        
        self.xyz_gradient_accum = torch.concat((self.xyz_gradient_accum, self.temp_xyz_gradient_accum),dim=0)
        self.color_gradient_accum = torch.concat((self.color_gradient_accum, self.temp_color_gradient_accum),dim=0)
        self.max_radii2D = torch.concat((self.max_radii2D, self.temp_max_radii2D),dim=0)
        self.denom = torch.concat((self.denom, self.temp_denom),dim=0)
        
    def training_one_frame_s3_setup(self, training_args):
        ...
        # Spawn
            ...
            # std가 작을수록(3DG 크기가 작을수록) 원래 3DG 근처에서 생성 / max : 1.09, min : 2.7e-12
            # std_scale default : 2
            stds = training_args.std_scale*self.get_scaling[selected_pts_mask].repeat(num_of_spawn,1)
            print(f'high grad stage 2: {selected_pts_mask.sum().item()}')
            #stds = 10*self.get_scaling[selected_pts_mask].repeat(num_of_spawn,1)
            #stds = torch.zeros_like(stds)
            #stds = self.get_scaling[selected_pts_mask].repeat(num_of_spawn,1)
            # min-max를 이용해 [0,1]로 mapping
            #stds = training_args.std_scale*((stds-torch.min(stds))/(torch.max(stds)-torch.min(stds)))
            ...
            #self._added_scaling = (self.scaling_inverse_activation(self.get_scaling[selected_pts_mask].repeat(num_of_spawn,1) / 0.8)).detach().requires_grad_(True)
